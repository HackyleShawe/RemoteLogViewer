...
public class LogWebSocketService {
    ...
    public void sendRealtimeLog(WebSocketSession wsSession) throws Exception {
        ...
        SessionDomain sessionDomain = buildWsSession4RealtimeLog(wsSession);

        //实时推送日志数据到客户端
        sendRealtimeLogToWebSocketClient(sessionDomain);
    }

    private SessionDomain buildWsSession4RealtimeLog(WebSocketSession wsSession) throws Exception {
        //检查当前WebSocket是否已经创建过连接
        SessionDomain existsWsSession = sessionDomainHolder.getSessionDomain(wsSession.getId());
        if(null != existsWsSession) {
            return existsWsSession;
        }

        SessionDomain sessionDomain = new SessionDomain();
        sessionDomain.setWebSocketSession(wsSession);
        ...
        sessionDomain.setLogTargetBean(targetBean);
        ...
        sessionDomain.setHistoryItems(historyItems);
        ...
        sessionDomain.setSshSession(sshSession);

        //缓存当前已经创建的连接
        sessionDomainHolder.addSessionDomain(wsSession.getId(), sessionDomain);
     ...
     *
     * @param sessionDomain 前端Client与后端WebSocketServer建立的连接实例
     */
    private void sendRealtimeLogToWebSocketClient(SessionDomain sessionDomain) throws Exception {
    ...
    /**
     * 发送根据关键字搜索日志数据给浏览器客户端
     * @param wsSession 前端Client与后端WebSocketServer建立的连接实例
     */
    public void sendSearchLog(WebSocketSession wsSession) throws Exception {
        //创建WebSocket与SSH连接，并缓存起来
        SessionDomain sessionDomain = buildWsSession4SearchLog(wsSession);

        //实时推送日志数据到客户端
        sendSearchLogToWebSocketClient(sessionDomain);
    }

    private SessionDomain buildWsSession4SearchLog(WebSocketSession wsSession) throws Exception {
        //检查当前WebSocket是否已经创建过连接
        SessionDomain existsWsSession = sessionDomainHolder.getSessionDomain(wsSession.getId());
        if(null != existsWsSession) {
            return existsWsSession;
        ...
        sessionDomain.setKeywords(keywords);
     ...
     *  4.获取WebSocket Session，只要它没有被关闭，就将日志数据通过该Session推送出去
     */
    private void sendSearchLogToWebSocketClient(SessionDomain sessionDomain) throws Exception {
        WebSocketSession wsSession = sessionDomain.getWebSocketSession();
        Session sshSession = sessionDomain.getSshSession();
        ...
        if(reader.readLine() == null) {
            wsSession.sendMessage(new TextMessage("没有数据！"));
        } else {
            String buf = "";
            while ((buf = reader.readLine()) != null) {
                if(wsSession.isOpen()) {
